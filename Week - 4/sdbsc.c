...
int compress_db(int fd)
{
    ...
    int orig_fd = open(DB_FILE, O_RDONLY);
    if (orig_fd == -1) {
        printf(M_ERR_DB_OPEN);
        return ERR_DB_FILE;
    }
    ...
    for (int id = MIN_STD_ID; id <= MAX_STD_ID; id++) {
        off_t offset = (off_t)id * STUDENT_RECORD_SIZE;
        if (lseek(orig_fd, offset, SEEK_SET) == -1) {
            continue;
        }

        student_t s;
        ssize_t bytes_read = read(orig_fd, &s, STUDENT_RECORD_SIZE);
        if (bytes_read == STUDENT_RECORD_SIZE && s.id == id && s.id != 0) {
            ...
            ssize_t bytes_written = write(tmp_fd, &s, STUDENT_RECORD_SIZE);
            if (bytes_written != STUDENT_RECORD_SIZE) {
                close(orig_fd);
                close(tmp_fd);
                printf(M_ERR_DB_WRITE);
