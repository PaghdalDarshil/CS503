...
int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        if (recv_bytes < 0) {  // Error in receiving data
            perror("Error receiving command from client");
            free(io_buff);
            return ERR_RDSH_COMMUNICATION;
        }
        
        if (recv_bytes == 0) {  // Client disconnected gracefully
        ...
        if (strcmp(io_buff, "stop-server") == 0) {
            printf("Client requested to stop server\n");
            free(io_buff);
            return OK_EXIT;
        }

        // Parse command into pipeline format
        ...
        // Execute command pipeline (redirect output to socket)
        rsh_execute_pipeline(cli_socket, &cmd_list);

        // Send EOF character to indicate end of output
