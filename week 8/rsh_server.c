...
int start_server(char *ifaces, int port, int is_threaded) {
    (void)is_threaded;  // Mark parameter as explicitly unused
...
int process_cli_requests(int svr_socket) {
    ...
    while (1) {
        ...
        // Stop server if `stop-server` command was received
        if (rc == OK_EXIT) {
...
int exec_client_requests(int cli_socket) {
    ...
    if (!io_buff) {
        perror("Error allocating io_buff");
        return ERR_RDSH_COMMUNICATION;
    }

    printf("[DEBUG] Allocated io_buff at %p\n", (void*)io_buff);
    ...
    while (1) {
        ...
        if (recv_bytes == 0) {  // Client disconnected
            printf("[DEBUG] Client disconnected, freeing io_buff at %p\n", (void*)io_buff);
            free(io_buff);
            return OK;
        }
        ...
        if (recv_bytes >= SH_CMD_MAX) {
            fprintf(stderr, "Error: Command too long (max %d characters).\n", SH_CMD_MAX);
            send_message_string(cli_socket, "Error: Command too long.\n");
            continue;
        }

        // Handle built-in commands
        if (strcmp(io_buff, "exit") == 0) {
            printf("[DEBUG] Client requested exit, freeing io_buff at %p\n", (void*)io_buff);
            free(io_buff);
            return OK;
        }
        if (strcmp(io_buff, "stop-server") == 0) {
            printf("[DEBUG] Client requested server shutdown, freeing io_buff at %p\n", (void*)io_buff);
            free(io_buff);
            return OK_EXIT;
        }

        // 🔹 Initialize `cmd_list` properly before use
        command_list_t cmd_list;
        memset(&cmd_list, 0, sizeof(cmd_list));  // 🔹 Clear memory before use

        int parse_status = build_cmd_list(io_buff, &cmd_list);
        if (parse_status == WARN_NO_CMDS) {
            send_message_string(cli_socket, "No command provided\n");
            continue;
        }
...
int send_message_string(int cli_socket, char *buff) {
    if (!buff) return ERR_RDSH_COMMUNICATION;  // Prevent sending NULL

    size_t total_bytes = strlen(buff);
    ssize_t bytes_sent = 0;

    while ((size_t)bytes_sent < total_bytes) {
        ssize_t sent = send(cli_socket, buff + bytes_sent, total_bytes - bytes_sent, 0);
        if (sent < 0) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] == 0) {  // Child Process
            ...
            if (i < clist->num - 1) dup2(pipes[i][1], STDOUT_FILENO);
                ...
                dup2(cli_sock, STDOUT_FILENO);  // Send stdout to client
                dup2(cli_sock, STDERR_FILENO);  // Send stderr to client
            }
