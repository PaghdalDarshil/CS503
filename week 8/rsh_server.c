...
int boot_server(char *ifaces, int port) {
    ...
    printf("Server listening on the %s:%d\n", ifaces, port);
    return svr_socket;
}
...
int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        if (recv_bytes == 0) {  
            printf("[DEBUG] Client is disconnected, freeing io_buff at %p\n", (void*)io_buff);
            free(io_buff);
            return OK;
        }
        io_buff[recv_bytes] = '\0';
        if (recv_bytes >= SH_CMD_MAX) {
            fprintf(stderr, "Error: Command  is too long (max %d characters).\n", SH_CMD_MAX);
            send_message_string(cli_socket, "Error: Command too long.\n");
            continue;
        }
        if (strcmp(io_buff, "exit") == 0) {
            printf("[DEBUG] Client has requested exit, freeing io_buff at %p\n", (void*)io_buff);
            free(io_buff);
            return OK;
        }
        if (strcmp(io_buff, "stop-server") == 0) {
            printf("[DEBUG] Client has requested server shutdown, freeing io_buff at %p\n", (void*)io_buff);
            free(io_buff);
            return OK_EXIT;
        }
        ...
        if (parse_status == WARN_NO_CMDS) {
            send_message_string(cli_socket, "No command is provided\n");
            continue;
        }
