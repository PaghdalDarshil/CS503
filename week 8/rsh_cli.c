...
int exec_remote_cmd_loop(char *server_ip, int port) {
    ...
    if (sock < 0) return ERR_RDSH_CLIENT;

    char *send_buff = malloc(SH_CMD_MAX);
    char *recv_buff = malloc(RDSH_COMM_BUFF_SZ);
    if (!send_buff || !recv_buff) return client_cleanup(sock, send_buff, recv_buff, ERR_MEMORY);

    while (1) {
        // Prompt user for command
        printf("%s", SH_PROMPT);
        if (!fgets(send_buff, SH_CMD_MAX, stdin)) break;

        // Remove newline and check if empty
        send_buff[strcspn(send_buff, "\n")] = '\0';
        if (strlen(send_buff) == 0) continue;

        // **Fix: Check command length before sending**
        if (strlen(send_buff) >= SH_CMD_MAX) {
            printf("Error: Command too long (max %d characters).\n", SH_CMD_MAX);
            continue;
        }

        // Send command to server
        if (send(sock, send_buff, strlen(send_buff) + 1, 0) < 0) {
        ...
        while (1) {
            ssize_t recv_size = recv(sock, recv_buff, RDSH_COMM_BUFF_SZ, 0);
            if (recv_size < 0) {
            ...
            int is_last_chunk = (recv_buff[recv_size - 1] == RDSH_EOF_CHAR);
            if (is_last_chunk) {
                recv_buff[recv_size - 1] = '\0';  // Remove EOF
                printf("%s", recv_buff);
                break;
            }
    ...
    return client_cleanup(sock, send_buff, recv_buff, OK);
}
